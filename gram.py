...
def diff_loss(model, targets, weights):
    diff_layers = []
    i=0
    for base, target in zip(model.outputs, targets):
        ...
        diff_layers.append(Lambda(lambda x, target=target: weights[i]*K.sum(K.square(target - x[0])),
...
def gram_loss_callable(gram_model, target_grams, shape, weights):
    ...
    to the image pixels'''
    loss = diff_loss(gram_model, target_grams, weights)
...
def synthesize_animation(pyramid_model, gram_model, target_grams,
        ...
        x0=None, max_iter=200, save_every=2, tol=1e-9, weights=[1,1]*4):
    ...
    print("gram model outputs:", len(gram_model.outputs))
    style_loss = diff_loss(gram_model, target_grams, weights)
